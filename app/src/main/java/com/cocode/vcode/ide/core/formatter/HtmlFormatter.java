...
public class HtmlFormatter extends BaseFormatter {
    ...
    private String reInject(String result, List<String> blocks, String uuid, String key) {
        for (int i = 0; i < blocks.size(); i++) {
            ...
            if (m.find()) {
                ...
                result = result.replace(m.group(0), indent(blocks.get(i), pad));
            }
        }
        return result;
    }
    ...
    private String formatHtml(String code, String uuid) {
        ...
        for (int ti = 0; ti < tokens.size(); ti++) {
            ...
            boolean isOpen = trimmed.startsWith("<") && !isClose && !trimmed.startsWith("<!--") && !trimmed.startsWith("<!");
            ...
            String tagName = "";
            if (isOpen || isClose) {
                tagName = tagName(trimmed).toLowerCase();
            }
